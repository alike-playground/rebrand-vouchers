...
The repo is sometimes edited directly on GitHub/Codespaces (`.devcontainer/` exists for that) — **pull before editing locally**. Deployment is Streamlit Cloud one-click from GitHub: `packages.txt` (apt system deps) + `requirements.txt` at repo root are all it needs. The live app (rebrand-vouchers-alike.streamlit.app, Parag's Streamlit account) deploys from **this** repo — pushes to `main` redeploy it. Re-platforming behind Cloudflare Access is planned as a later update.
...
**Data model** (the contract between all modules): a dict `{"trip": {...}, "contacts": {...}}`. `trip` holds `booking_id, destination, nights, travel_date_display, pax_display, guest_lead, guests[], arrival/departure ({mode,date,time,remarks} or None), hotels[], days[], terms[]`. Each day is `{n, date_display, location, stops:[{title, pickup, pickup_time, drop, remarks, description, thumbs[], tour_type?, drop_time?}], leisure}` (`tour_type` = "Private"/"SIC (Shared)"/"Tickets Only", rendered as a badge beside the stop title; `drop_time` = SIC return-pickup time — both added V3.7, both optional/absent on stops that lack them). `samples/jash_data.json` is the reference example. `extract()` also returns `_ocr_warnings` and `_ocr_pages_raw` keys that `app.py` pops off before storing.

**extractor.py** — the source TravClan PDFs have *no usable text layer* (Qt-rendered, Identity-H encoding), so every page is rasterized at 300 DPI and OCR'd with Tesseract (`ocr_pdf`), then parsed with per-section regex heuristics (`parse_booking_meta`, `parse_guests`, `parse_flights`, `parse_hotels`, `parse_days`, `parse_terms`). Stop titles match `<name> | <type>` where type is PRIVATE / SIC / TICKETS ONLY / SEAT-IN-COACH / `N Cab(s)` (`STOP_TYPE` — extend it when a new voucher format uses a new type, and mirror the keyword in `_match_thumbs_to_stops`'s `STOP_TOKENS`). `parse_terms` picks between bullet-based and paragraph-based splitting: paragraphs win when they yield ≥50% more items (OCR often drops bullet glyphs — the Sachin/Rajendra cases). Embedded hotel/activity thumbnails are pulled with PyMuPDF (`extract_embedded_images`, min 200px side to skip decorative icons) and matched to itinerary stops by y-position using Tesseract word bounding boxes (`_match_thumbs_to_stops` — best-effort, failures are swallowed). `_sanity_check` produces the ops-facing warnings. When a new TravClan format breaks parsing, the raw OCR text is exposed in the UI ("Show raw OCR text" expander) so the parser can be taught the format. V3.7 (UAT patch, Singapore vouchers takcko/tayk1v — see `CHANGELOG_V3.7.md`) hardened three spots without new deps: the `Day N:` weekday is now optional (tentative vouchers read "Day 1: 05 Aug 2026" with no weekday), `parse_terms` strips the leading OCR'd bullet glyph (Tesseract renders "•" inconsistently as e/*/_/+/,), and SIC stops now surface `tour_type` + `drop_time` (with `pickup_time` truncated before the trailing "Return Pick up time").
